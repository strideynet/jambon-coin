...
# docker buildx build --tag jamboncoin --file .pi/Dockerfile --platform linux/arm64 --cache-from jamboncoin:cache --cache-to jamboncoin:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar jamboncoin
...
sudo bash -c "echo jamboncoin > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/jamboncoin.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/jamboncoin.img
